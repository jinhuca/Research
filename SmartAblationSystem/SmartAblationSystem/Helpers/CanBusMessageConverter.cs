    ...
    public static class CanBusMessageConverter
    {
        ...
        public static double ConverteNegativDecimalData(byte[] data, int index)
        {
            ...
            *******************************************************/
            ...
            // these code is used to calculate the negative values
            return (((data[index] * 256 + data[index + 1] << 16)) >> 16) / 10.0;
        }
        ...
        /// </summary>
        /// <param name="data">A bytes array representing the Can Bus data to convert.</param>
        /// <param name="index">the start index of byte[] </param>
        /// <returns>An integer representing the converted value.</returns>
        public static UInt32 ConvertDataToUInt32(byte[] data, int index)
        {
          return (UInt32)(data[index] << 24) | (UInt32)(data[index+1] & 0xFF) << 16 | (UInt32)(data[index+2] & 0xFF) << 8 | (UInt32)(data[index+3] & 0xFF);
        }
        ...
        public static double ConvertDecimalDataWithFactor(byte[] data, int index, double factor)
        {
