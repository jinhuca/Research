...
{
  public class PSMessageConfig
  {
    public int Interval { get; set; }
    public IDictionary<string, StateToPSValue> StateToPSMap { get; } = new Dictionary<string, StateToPSValue>();
    ...
    private bool ParseSettingsByState(XmlNode node)
    {
      ...
      try
      {
        var PSstateNode = node?.SelectNodes("//Settings/PSstate");
        if (PSstateNode == null) return false;

        foreach (XmlNode statenode in PSstateNode)
        {
          string state;
          StateToPSValue value = new StateToPSValue();
          ...
          foreach (XmlNode setting in statenode.ChildNodes)
          {
              ...
              && nameInnerText != null)
            {
              try
              {
                ...
                {
                  case "PS1":
                    value.PS1 = double.Parse(valueInnerText, CultureInfo.InvariantCulture);
                    break;
                  case "PT5":
