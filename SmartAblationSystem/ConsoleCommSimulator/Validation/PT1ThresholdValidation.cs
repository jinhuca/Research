...
{
  public class PT1ThresholdValidation : ThresholdValidation
  {
    // dictionary specific to PT
    private Dictionary<MessageStateId, Thresholds> pt1thresholdDictionary;
    private CMCUStatusError _previouslyHadError;
    public PT1ThresholdValidation(IEventAggregator eventAggregator) : base(eventAggregator)
    {
      // Initialize the internal dictionary for PTValidator
      pt1thresholdDictionary = new Dictionary<MessageStateId, Thresholds>();
    }
    public override void HandleThresholdUpdateEvent(UpdateThresholdEventArgs e)
    {
      ...
      if (e.ThresholdName == ThresholdType.PT1)
      {
        UpdateThresholdDictionary(e.ThresholdDictionary, pt1thresholdDictionary);
      }
    }
    ...
    public override void ValidateThresholds(double currentValue, MessageStateId currentStateId)
    {
      ...
      try
      {
        ...
        if (pt1thresholdDictionary.ContainsKey(currentStateId))
        {
          if (currentValue > pt1thresholdDictionary[currentStateId].FailValue)
          {

            PTErrors = CMCUStatusError.PressurePT1InTankIsTooHigh;
            _previouslyHadError = PTErrors;
            // switch to exception state
            UpdateStateToException();
          }
          else if (currentValue < pt1thresholdDictionary[currentStateId].LowValue)
          {
            PTErrors = CMCUStatusError.PressurePT1InTankIsLow;
            _previouslyHadError = PTErrors;
          }
        }
