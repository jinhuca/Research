...
internal sealed class RAMSPDToolkitDriver : IPawnIODriver {
  ...
  public IPawnIOModule LoadModule(PawnIOSMBusIdentifier pawnIOSMBusIdentifier) {
    ...
    try {
      var pawnIO = PawnIo.PawnIo.LoadModuleFromResource(typeof(RAMSPDToolkitDriver).Assembly, $"{nameof(CrystalMonitor)}.Resources.PawnIo.{moduleResourceFilename}");
