...
namespace Crystal
{
  public partial class CrystalContainer : ICrystalContainer
  {
    #region Fields
    ...
    /// <inheritdoc />
    ICrystalContainer ICrystalContainer.RegisterType(Type typeFrom, Type typeTo, string name, ITypeLifetimeManager lifetimeManager, InjectionMember[] injectionMembers)
    {
    ...
    /// <inheritdoc />
    ICrystalContainer ICrystalContainer.RegisterInstance(Type type, string name, object instance, IInstanceLifetimeManager lifetimeManager)
    {
    ...
    /// <inheritdoc />
    public ICrystalContainer RegisterFactory(Type type, string name, Func<ICrystalContainer, Type, string, object> factory, IFactoryLifetimeManager lifetimeManager)
    {
      LifetimeManager manager = (null != lifetimeManager)
                              ? (LifetimeManager)lifetimeManager
...
#pragma warning restore CS0618
    ...
    /// <inheritdoc />
    bool ICrystalContainer.IsRegistered(Type type, string name) => ReferenceEquals(All, name) ? IsTypeExplicitlyRegistered(type)
    ...
    /// <inheritdoc />
    object ICrystalContainer.Resolve(Type type, string name, params ResolverOverride[] overrides)
    {
      // Verify arguments
      if (null == type) throw new ArgumentNullException(nameof(type));
      ...
      var container = registration.Get(typeof(LifetimeManager)) is ContainerControlledLifetimeManager manager
                    ? (CrystalContainer)manager.Scope
                    : this;
    ...
    /// <inheritdoc />
    object ICrystalContainer.BuildUp(Type type, object existing, string name, params ResolverOverride[] overrides)
    {
      // Verify arguments
      if (null == type) throw new ArgumentNullException(nameof(type));
    ...
    /// <inheritdoc />
    ICrystalContainer ICrystalContainer.CreateChildContainer()
    {
      var child = new CrystalContainer(this);
      ChildContainerCreated?.Invoke(this, new ChildContainerCreatedEventArgs(child._context));
      return child;
    }

    /// <inheritdoc />
    ICrystalContainer ICrystalContainer.Parent => _parent;
