...
namespace Crystal.Injection
{
  public static class Validating
  {
    public static Func<Type, InjectionMember, ConstructorInfo> ConstructorSelector = (Type type, InjectionMember member) =>
    {
    ...
    public static Func<Type, InjectionMember, MethodInfo> MethodSelector = (Type type, InjectionMember member) =>
    {
      ...
      foreach (var info in type.GetDeclaredMethods())
      {
        ...
        if (null != selection)
        {
          throw new ArgumentException($"Method {method.Name}({method.Data.Signature()}) is ambiguous, it could be matched with more than one method on type {type?.Name}.");
        }
        selection = info;
      }
      ...
      if (null == selection)
      {
        throw new ArgumentException($"Injected method {method.Name}({method.Data.Signature()}) could not be matched with any public methods on type {type?.Name}.");
      }

      if (selection.IsStatic)
      {
        throw new ArgumentException($"Static method {method.Name} on type '{selection.DeclaringType.Name}' cannot be injected");
      }

      if (selection.IsPrivate)
      {
        throw new InvalidOperationException($"Private method '{method.Name}' on type '{selection.DeclaringType.Name}' cannot be injected");
      }

      if (selection.IsFamily)
      {
        throw new InvalidOperationException($"Protected method '{method.Name}' on type '{selection.DeclaringType.Name}' cannot be injected");
      }

      if (selection.IsGenericMethodDefinition)
      {
        throw new ArgumentException($"Open generic method {method.Name} on type '{selection.DeclaringType.Name}' cannot be injected");
      }
      ...
      {
        throw new ArgumentException($"Method {method.Name} on type '{selection.DeclaringType.Name}' cannot be injected. Methods with 'ref' parameters are not injectable.");
      }
    ...
    public static Func<Type, InjectionMember, FieldInfo> FieldSelector = (Type type, InjectionMember member) =>
    {
      FieldInfo selection = null;
      var field = (InjectionMember<FieldInfo, object>)member;
      if (field.IsInitialized) throw new InvalidOperationException("Sharing an InjectionField between registrations is not supported");
      ...
      if (null == selection)
      {
        throw new ArgumentException($"Injected field '{field.Name}' could not be matched with any public field on type '{type?.Name}'.");
      }

      if (selection.IsStatic)
      {
        throw new InvalidOperationException($"Static field '{selection.Name}' on type '{type?.Name}' cannot be injected");
      }

      if (selection.IsInitOnly)
      {
        throw new InvalidOperationException($"Readonly field '{selection.Name}' on type '{type?.Name}' cannot be injected");
      }

      if (selection.IsPrivate)
      {
        throw new InvalidOperationException($"Private field '{selection.Name}' on type '{type?.Name}' cannot be injected");
      }

      if (selection.IsFamily)
      {
        throw new InvalidOperationException($"Protected field '{selection.Name}' on type '{type?.Name}' cannot be injected");
      }

      if (!field.Data.Matches(selection.FieldType))
      {
        throw new ArgumentException($"Injected data '{field.Data}' could not be matched with type of field '{selection.FieldType.Name}'.");
      }
    ...
    public static Func<Type, InjectionMember, PropertyInfo> PropertySelector = (Type type, InjectionMember member) =>
    {
