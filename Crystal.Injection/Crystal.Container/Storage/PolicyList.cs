...
namespace Crystal.Storage
{
  ...
  public class PolicyList : IPolicyList
  {
    ...
    public object Get(Type type, Type policyInterface)
    {
      ...
      if (_policies?.TryGetValue(new PolicyKey(type, CrystalContainer.All, policyInterface), out policy) ?? false)
      {
        return policy;
      }

      return _innerPolicyList?.Get(type, CrystalContainer.All, policyInterface);
    }
    ...
    public void Set(Type type, Type policyInterface, object policy)
    {
      ...
      _policies[new PolicyKey(type, CrystalContainer.All, policyInterface)] = policy;
    }
    ...
    private struct PolicyKey
    {
      ...
      public PolicyKey(Type type, string name, Type policyType)
      {
        ...
        _hash = (policyType?.GetHashCode() ?? 0) * 37 +
            (ReferenceEquals(CrystalContainer.All, name) ? type?.GetHashCode() ?? 0
