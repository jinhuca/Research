...
namespace Crystal.Strategies
{
  ...
  public class LifetimeStrategy : BuilderStrategy
  {
          ...
          if (null == policy)
          {
            ...
            if (policy is IDisposable)
            {
              var scope = policy is ContainerControlledLifetimeManager container
                        ? ((CrystalContainer)container.Scope)?.LifetimeContainer ?? context.Lifetime
                        : context.Lifetime;
              scope.Add(policy);
            }
          }
    ...
    public override bool RequiredToBuildType(ICrystalContainer container, Type type, InternalRegistration registration, params InjectionMember[] injectionMembers)
    {
    ...
    public override bool RequiredToResolveInstance(ICrystalContainer container, InternalRegistration registration) => true;
