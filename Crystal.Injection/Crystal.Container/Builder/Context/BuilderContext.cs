...
namespace Crystal.Builder
{
  ...
  public struct BuilderContext : IResolveContext
  {
    ...
    public ICrystalContainer Container => Lifetime?.Container;
    ...
    public object Resolve(Type type, string name)
    {
      ...
      return Resolve(type, name, (InternalRegistration)((CrystalContainer)Container).GetRegistration(type, name));
    }
    ...
    public object Get(Type type, string name, Type policyInterface)
    {
      return List.Get(type, name, policyInterface) ??
             (type != RegistrationType || name != Name
                 ? ((CrystalContainer)Container).GetPolicy(type, name, policyInterface)
                 : Registration.Get(policyInterface));
    }

    public object Get(Type type, Type policyInterface)
    {
      return List.Get(type, CrystalContainer.All, policyInterface) ??
             ((CrystalContainer)Container).GetPolicy(type, CrystalContainer.All, policyInterface);
    }
    ...
    {
      List.Set(type, CrystalContainer.All, policyInterface, policy);
    }
    ...
    public object Resolve(Type type, string name, InternalRegistration registration)
    {
      unsafe
      {
        ...
        var container = registration.Get(typeof(LifetimeManager)) is ContainerControlledLifetimeManager manager
                      ? ((CrystalContainer)manager.Scope).LifetimeContainer
                      : Lifetime;
