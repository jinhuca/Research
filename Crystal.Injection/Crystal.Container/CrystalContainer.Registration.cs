...
namespace Crystal
{
  public partial class CrystalContainer
  {
    ...
    private bool IsExplicitlyRegisteredLocally(Type type, string name)
    {
      ...
      for (var i = _registrations.Buckets[targetBucket]; i >= 0; i = _registrations.Entries[i].Next)
      {
        ...
        return registry?[name] is ContainerRegistration ||
               (((ICrystalContainer)_parent)?.IsRegistered(type, name) ?? false);
      }

      return ((ICrystalContainer)_parent)?.IsRegistered(type, name) ?? false;
    }
    ...
    private static RegistrationSet GetRegistrations(CrystalContainer container)
    {
      ...
      int GetStartIndex()
      {
        ...
        while (++start < length)
        {
          if (typeof(ICrystalContainer) != container._registrations.Entries[start].Key)
            continue;
          return start;
        }
    ...
    private static RegistrationSet GetRegistrations(CrystalContainer container, params Type[] types)
    {
    ...
    private static RegistrationSet GetNamedRegistrations(CrystalContainer container, params Type[] types)
    {
