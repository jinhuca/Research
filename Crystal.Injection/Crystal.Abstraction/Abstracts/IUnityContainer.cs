...
{
  /// <summary>
  /// <para>Interface defining the behavior of the Crystal dependency injection container.</para>
  ...
  /// </summary>
  [CLSCompliant(true)]
  public interface ICrystalContainer : IDisposable
  {
    /// <summary>
    ...
    /// </para>
    /// <para>
    /// During registration, Crystal performs only a limited number of checks. To enable slower but more thorough and detailed validation add 
    ...
    /// <seealso cref="Inject"/>
    /// <seealso cref="Invoke"/>
    /// <seealso cref="Crystal.Resolve"/>
    /// <seealso cref="TypeLifetime"/>
    ...
    ICrystalContainer RegisterType(Type registeredType, Type mappedToType, string name, ITypeLifetimeManager lifetimeManager, params InjectionMember[] injectionMembers);
    ...
    /// <returns>The <see cref="ICrystalContainer"/> object that this method was called on.</returns>
    ICrystalContainer RegisterInstance(Type type, string name, object instance, IInstanceLifetimeManager lifetimeManager);
    ...
    /// <returns>The <see cref="ICrystalContainer"/> object that this method was called on.</returns>
    ICrystalContainer RegisterFactory(Type type, string name, Func<ICrystalContainer, Type, string, object> factory, IFactoryLifetimeManager lifetimeManager);
    ...
    /// and has a very small overhead. It is an order of magnitude faster than querying <see cref="ICrystalContainer.Registrations"/>
    /// collection.
    /// </para>
    /// </remarks>
    ...
    /// </para>
    /// <para>
    /// If this is a child container <see cref="ICrystalContainer.Registrations"/> will contain all registrations from this container 
    ...
    /// <remarks>
    /// <para>
    /// During resolution Crystal checks if <see cref="Type"/> is registered and uses that registration to create an object. 
    ...
    /// </para>
    /// <para>
    /// During resolution, Crystal performs only a limited number of checks. If any errors occur, error information is very brief.
    ...
    /// </summary>
    /// <remarks>
    /// Crystal allows creating scopes with the help of child container. A child container shares the 
    /// parent's configuration but can be configured with different settings or lifetime.</remarks>
    /// <returns>The new child container.</returns>
    ICrystalContainer CreateChildContainer();
  }
}
