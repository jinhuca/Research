...
{
  /// <summary>
  /// Interface defining the behavior of the Crystal dependency injection container.
  /// </summary>
  [CLSCompliant(true)]
  public interface ICrystalContainerAsync : IDisposable
  {
    /// <summary>
    ...
    /// <returns>Returns <see cref="Task"/> indicating when registration is done</returns>
    Task RegisterFactory(IEnumerable<Type> interfaces, string name, Func<ICrystalContainer, Type, string, object> factory, IFactoryLifetimeManager lifetimeManager);
    ...
    ICrystalContainerAsync Parent { get; }
    ...
    ICrystalContainerAsync CreateChildContainer();
  }
}
