...
namespace Crystal.Graphics
{
  ...
  public class MeshBuilder
  {
    ...
    public void AddCylinder(Point3D p1, Point3D p2, double radius = 1, int thetaDiv = 32, bool cap1 = true, bool cap2 = true)
    {
      ...
      var l = SharedFunctions.Length(ref n);
      n.Normalize();
      AddCone(p1, n, radius, radius, l, cap1, cap2, thetaDiv);
    }
    ...
    public void AddPolygon(IList<Point> points, Vector3D axisX, Vector3D axisY, Point3D origin)
    {
      ...
      if(indices != null)
      {
        ...
        foreach(var p in points)
        {
          positions.Add(origin + (axisX * p.X) + (axisY * p.Y));
        }
    ...
    public void AddRectangularMesh(IList<Point3D> points, int columns, bool flipTriangles = false)
    {
      ...
      if(flipTriangles)
      {
        AddRectangularMeshTriangleIndicesFlipped(index0, rows, columns);
      }
      else
      {
        AddRectangularMeshTriangleIndices(index0, rows, columns);
    ...
    {
      var p0 = positions.Count;
      Append(GetUnitSphere(subdivisions));
      var p1 = positions.Count;
      for(var i = p0; i < p1; i++)
      {
    ...
    public void AddTorus(double torusDiameter, double tubeDiameter, int thetaDiv = 36, int phiDiv = 24)
    {
      ...
      // If the second Diameter is zero, we can't build out torus
        ...
        throw new CrystalException("Torus must have a Diameter bigger than 0");
        ...
        for(var i = 0; i < thetaDiv; i++)
        {
          ...
          if(!selfIntersecting)
          {
            ...
            for(var j = 0; j < phiDiv; j++)
            {
              ...
              triangleIndices.Add(firstPointIdxNextCircle + j + positionsCount);

              triangleIndices.Add(firstPointIdxNextCircle + j + positionsCount);
              triangleIndices.Add(firstPointIdx + jNext + positionsCount);
              triangleIndices.Add(firstPointIdxNextCircle + jNext + positionsCount);
            }
          }
        ...
        IList<Point> section, bool isTubeClosed, bool isSectionClosed, bool frontCap = false, bool backCap = false)
    {
      ...
      if(frontCap || backCap && path.Count > 1)
      {
        ...
        if(frontCap)
        {
          ...
          AddTriangleFan(circleFront, normals, fanTextures);
        }
      }
    }
    ...
    public void ChamferCorner(Point3D p, double d, double eps = 1e-6f, IList<Point3D> chamferPoints = null)
    {
      ...
      for(var i = 0; i < ntri; i += 3)
      {
        ...
        positions[triangleIndices[i01]] = p01.Value;

        var i02 = positions.Count;
        positions.Add(p02.Value);
    ...
    public MeshGeometry3D? ToMesh(bool freeze = false)
    {
      ...
      if(normals != null)
      {
        mg.Normals = new Vector3DCollection(normals);
      }

      if(textureCoordinates != null)
      {
        mg.TextureCoordinates = new PointCollection(textureCoordinates);
      }
