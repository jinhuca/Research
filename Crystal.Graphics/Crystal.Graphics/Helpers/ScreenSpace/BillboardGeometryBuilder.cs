  ...
  public class BillboardGeometryBuilder : ScreenGeometryBuilder
  {
    ...
    public Point3DCollection GetPositions(IList<Billboard> billboards, Vector offset)
    {
      ...
      foreach(var bb in billboards)
      {
        ...
        {
          var viewPoint = (Point4D)bb.Position * visualToProjection;
          screenPoint = new Point4D(viewPoint.X, viewPoint.Y, viewPoint.Z + bb.WorldDepthOffset, viewPoint.W) * projectionToScreen;
        }
        else
        {
        ...
        p = new Point4D(spx + ((bb.Right + offset.X) * spw), spy + ((bb.Bottom + offset.Y) * spw), spz, spw) * screenToVisual;
        wi = 1 / p.W;
        positions.Add(new Point3D(p.X * wi, p.Y * wi, p.Z * wi));

        // Convert top-right corner to visual space
        p = new Point4D(spx + ((bb.Right + offset.X) * spw), spy + ((bb.Top + offset.Y) * spw), spz, spw) * screenToVisual;
        wi = 1 / p.W;
        positions.Add(new Point3D(p.X * wi, p.Y * wi, p.Z * wi));

        // Convert top-left corner to visual space
        p = new Point4D(spx + ((bb.Left + offset.X) * spw), spy + ((bb.Top + offset.Y) * spw), spz, spw) * screenToVisual;
        wi = 1 / p.W;
        positions.Add(new Point3D(p.X * wi, p.Y * wi, p.Z * wi));
      }
